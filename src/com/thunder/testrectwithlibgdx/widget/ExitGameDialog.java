...
public class ExitGameDialog {
	 ...
	 * @param context
	 */
	public ExitGameDialog(SmallRectView smallRectView) {
		super();
	...
	private void init() {
		TextureRegion textureRegion = new TextureRegion(new Texture(
				Gdx.files.internal("res/img/exit_game_dialog.png")), 58, 32, 392, 188);
	...
	private void addButton() {
		...
		button_cancel = new Button(new TextureRegion(new Texture(
				Gdx.files.internal("res/img/button_cancel.png")), 46, 26, 160, 72));
		button_cancel.x = 210;
		button_cancel.y = 20;
		button_cancel.width = 135;
		button_cancel.height = 55;
		
		/* 取消按键  */
		button_cancel.setClickListener(new ClickListener() {
			@Override
			public void click(Actor arg0, float arg1, float arg2) {
				// 恢复游戏状态
				GameConstant.mGameState = mSmallRectView.mOldState; 
				// 移除对话框
				mStage.removeActor(dialog);
			}
		});
		dialog.addActor(button_ok);
		dialog.addActor(button_cancel);
	}
}
